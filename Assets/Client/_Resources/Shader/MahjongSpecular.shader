...
Shader "XH/MahjongSpecular" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _DiffuseMask; uniform float4 _DiffuseMask_ST;
            struct VertexInput {
...
////// Emissive:
                float4 _DiffuseMask_var = tex2D(_DiffuseMask,TRANSFORM_TEX(i.uv0, _DiffuseMask));
                float node_9574_if_leA = step(_DiffuseMask_var.a,0.1);
                float node_9574_if_leB = step(0.1,_DiffuseMask_var.a);
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float3 node_544 = (_Diffuse_var.rgb*_Color.rgb); // Diffuse Color
                float3 node_2460 = (node_544*UNITY_LIGHTMODEL_AMBIENT.rgb); // Ambient Light
                float3 emissive = lerp((node_9574_if_leA*node_2460)+(node_9574_if_leB*node_544),node_2460,node_9574_if_leA*node_9574_if_leB);

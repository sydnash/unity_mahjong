...
namespace gcloud_voice
{
    ...
    public override event MuteSwitchResultHandler OnMuteSwitchState;
		...
		{
			Debug.Log("GCloudVoice_C# API: getAudioDeviceConnectionState");
			if (!bInit)
			{
			...
			Debug.Log("GCloudVoice_C# API: getAudioDeviceConnectionState  nRet=" + nRet);
			return nRet;
		}

        public override void EnableBluetoothSCO(bool enable)
		{
			Debug.Log("GCloudVoice_C# API: EnableBluetoothSCO");
			if (!bInit)
			{
				return;
			}
        ...
        {
            Debug.Log("GCloudVoice_C# API: CheckDeviceMuteState");
            if (!bInit)
            {
                return (int)GCloudVoiceDeviceState.AUDIO_DEVICE_UNCONNECTED;
            }

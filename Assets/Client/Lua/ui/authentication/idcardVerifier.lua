...
local function isBirthDate(date)
    ...
    if year < 1900 or year > 2019 or month >12 or month < 1 then
        return false
    end
    -- //月份天数表
...
local function checkSum(idcard)
    local sc = sumCode(idcard)
    return sc == idcard:sub(18,18 )
end
...
local function verifyIDCard(idcard)
    ...
    -- 将15位身份证号扩展为18位
    if len == 15 then
        local f = string.sub(idcard, 1, 6) .. "19"
        local t = f .. string.sub(idcard, 7, 15)
        local s = sumCode(t)
